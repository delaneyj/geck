...
func NewWorld() *World {
	...
	internalID := w.CreateEntity(InternalName)
			if internalID != InternalID {
				panic("id mismatch")
			}
		wildcardID := w.CreateEntity(WildcardName)
			if wildcardID != WildcardID {
				panic("id mismatch")
			}
		childOfID := w.CreateEntity(ChildOfName)
			if childOfID != ChildOfID {
				panic("id mismatch")
			}
		instanceOfID := w.CreateEntity(InstanceOfName)
			if instanceOfID != InstanceOfID {
				panic("id mismatch")
			}
		componentID := w.CreateEntity(ComponentName)
			if componentID != ComponentID {
				panic("id mismatch")
			}
		tagID := w.CreateEntity(TagName)
			if tagID != TagID {
				panic("id mismatch")
			}
	...
	registerComponent(w, PositionResetValue, PositionName)
			registerComponent(w, VelocityResetValue, VelocityName)
			registerComponent(w, RotationResetValue, RotationName)
			return w
}
...
func (w *World) MarshalAll() () {
	w.MarshalAllIdentifier()
		w.MarshalAllName()
		w.MarshalAllInternal()
		w.MarshalAllWildcard()
		w.MarshalAllChildOf()
		w.MarshalAllInstanceOf()
		w.MarshalAllComponent()
		w.MarshalAllTag()
		w.MarshalAllPosition()
		w.MarshalAllVelocity()
		w.MarshalAllRotation()
		}